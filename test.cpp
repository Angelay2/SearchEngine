/*
1. 一个搜索结果中, 至少需要包含以下四个部分:
标题, 描述, 展示url, 点击url

搜索结果内容 ,是需要和用户输入的查询词之间存在一定的"关联关系" ---> 相关性
...
4. 服务器模块(提供一个http服务器供浏览器直接访问, 给外部提供服务)
...
/*
1. 分词的实现
C++开源的分词库, 使用cppjieba这样的库, 先下载到本地
vim这个东西实在是太老了, 功能也跟不上时代了 
code-server: 网页版本的VScode(可以吧code-server部署到云服务器上, 通过浏览器就可以打开, 随时进行开发,(突破图形截面的限制)
中文分词往往依赖一些词典, (词典就是把一些词放到了一起)
从github上下载jieba分词时, 就包含了相关的词典文件

makefile中必须通过 -I(大写i)来指定结巴分词的头文件在哪个目录中
这里的这个路径就必须和jieba分词下载的路径一致
